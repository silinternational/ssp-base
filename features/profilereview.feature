Feature: Prompt to review profile information
  ...
  Scenario: Present profile review as required by the user profile
    ...
    # TODO: replace the following with unit tests. As an integration test, these are cumbersome because the review
    # status is reset upon the first use of a each test user.
...
#    Then I should end up at my intended destination
  ...
  # FIXME: this uses the `profile_review` user, which is used elsewhere, and the review flag gets cleared before this
  #  scenario runs.
