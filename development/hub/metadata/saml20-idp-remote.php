<?php
/**
 * SAML 2.0 remote IdP metadata for SimpleSAMLphp.
 *
 * Remember to remove the IdPs you don't use from this file.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-idp-remote
 */

/*
 * IdP 1
 */
$metadata['https://ssp-idp1.local'] = [
    'metadata-set' => 'saml20-idp-remote',
    'entityid' => 'https://ssp-idp1.local',
    'name' => [
        'en' => 'IDP 1',
    ],
    'IDPNamespace' => 'IDP-1',
    'logoCaption' => 'IDP-1 staff',
    'logoURL' => 'https://dummyimage.com/125x125/0f4fbd/ffffff.png&text=IDP+1',

    'description' => 'Local IDP for testing SSP Hub (default port)',

    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp1.local/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp1.local/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'certData' => '',
];

/*
 * IdP 2
 */
$metadata['https://ssp-idp2.local'] = [
    'metadata-set' => 'saml20-idp-remote',
    'entityid' => 'https://ssp-idp2.local',
    'name' => [
        'en' => 'IDP 2',
    ],
    'IDPNamespace' => 'IDP-2',
    'logoCaption' => 'IDP-2 staff',
    'logoURL' => 'https://dummyimage.com/125x125/0f4fbd/ffffff.png&text=IDP+2',

    'description' => 'Local IDP2 for testing SSP Hub (normal port)',

    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp2.local/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp2.local/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'certData' => '',

    // limit which Sps can use this IdP
    'SPList' => ['https://ssp-sp1.local', 'https://ssp-sp2.local', 'https://ssp-sp4.local', 'https://ssp-sp5.local'],
];

/*
 * IdP 3
 */
$metadata['https://ssp-idp3.local'] = [
    'metadata-set' => 'saml20-idp-remote',
    'entityid' => 'https://ssp-idp3.local',
    'name' => [
        'en' => 'IDP 3',
    ],
    'IDPNamespace' => 'IDP-3',
    'logoCaption' => 'IDP-3 staff',
    'logoURL' => 'https://dummyimage.com/125x125/0f4fbd/ffffff.png&text=IDP+3',

    'description' => 'Local IDP3 for testing SSP Hub',

    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp3.local/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp3.local/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'certData' => '',
];

/*
 * IdP 4
 */
$metadata['https://ssp-idp4.local'] = [
    'metadata-set' => 'saml20-idp-remote',
    'entityid' => 'https://ssp-idp4.local',
    'name' => [
        'en' => 'IDP 4',
    ],
    'IDPNamespace' => 'IDP-4',
    'logoCaption' => 'IDP-4 staff',
    'logoURL' => 'https://dummyimage.com/125x125/0f4fbd/ffffff.png&text=IDP+4',

    'description' => 'Local IDP4 for testing SSP Hub',

    'SingleSignOnService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp4.local/saml2/idp/SSOService.php',
        ],
    ],
    'SingleLogoutService' => [
        [
            'Binding' => 'urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect',
            'Location' => 'https://ssp-idp4.local/saml2/idp/SingleLogoutService.php',
        ],
    ],
    'certData' => '',
];
